...
- **3D graph** — every wiki page (`concept` / `entity`), every frontmatter tag (`#tag`, its own node like in Obsidian), every cited raw source, cross-vault references, and unresolved wikilinks (dim, like Obsidian). Node size follows the chosen hub metric; hover to see a node's neighbors; click to open it; double-click to fly to it. Right-click/drag to orbit, wheel to zoom.
- **Hub filter** — pick what "hub" means: **bridging** (betweenness centrality, recomputed on the nodes currently visible), **cross-cluster links** (edges into other Louvain communities — the nodes that stitch clusters together), **connections** (in + out, Obsidian's sizing), **inbound only**, or **cut vertices** (articulation points). "Hide top N hubs" removes them and their edges, whatever their kind (a heavily-linked note counts like a popular tag), so the islands they glue together become visible; the panel reports how many islands and isolated nodes result. **Colour by community** (Louvain) shows clusters even while everything is still connected; **colour by island** paints connected components after hiding. A ranked top-hubs list lets you hide/show individual hubs; per-kind checkboxes scope the filter. **✨ Reveal clusters** is a one-click preset (hide tag/raw/unresolved nodes, hide the top 30 cross-cluster hubs, colour by community) — tags are usually the mesh that hides a vault's structure.
- **Mermaid** — ```` ```mermaid ```` fences in a page render as diagrams (mermaid.js vendored, `securityLevel: strict`).
...
| `CLAUDE_MAX_API_PROXY_ENDPOINT` | Overrides [`claude-max-api-proxy`](https://github.com/sethschnrt/claude-max-api-proxy)'s default `http://localhost:3456/v1` (`claude-max-api-proxy/<model>`, e.g. `claude-max-api-proxy/claude-sonnet-4` — no key needed; a local OpenAI-compatible proxy that serves requests through an already-authenticated `claude` CLI session, i.e. your Claude Pro/Max subscription instead of per-token API billing) |
