...
A retrospective of what `docs/okf-pipeline-design.md` and `docs/okf-mcp-implementation-plan.md` failed to specify clearly enough — and what shipped as a bug or a missing feature as a direct result. Spans the project's full release history (`v0.2.0` onward), not just the most recent batch. Each entry pairs **what was missing** with **the fix/feature it took**, and closes with a **lesson** meant to transfer to *other* projects' planning docs, not just this one.

Update this file in tandem with [CHANGELOG.md](../CHANGELOG.md): whenever a future bug traces back to an ambiguous or absent design-doc requirement, add an entry here alongside the CHANGELOG entry for the fix — as its own entry, not filed under a version-number heading; the `Fixed in` line on each entry carries that instead.

### Gap: example config value not given in the form the code actually required
**Fixed in:** `v0.2.1`
**What was missing:** `src/core/vault_config.rs`'s own doc-comment example (mirroring the design doc's style) shows `[providers.ollama] base_url = "http://localhost:11434"` — deliberately without a trailing slash — with nothing stating that the HTTP client this value feeds into requires one.
**What it took:** `compiler::provider::execute_compile_prompt` built its request URL by direct string concatenation against whatever `base_url` was resolved, producing a malformed URL whenever the default (or a copy-pasted example) omitted the trailing slash. Fixed by normalizing scheme + trailing slash once, generically, at the single point every provider's base URL flows through.
**Lesson:** when a design doc's example value will later be concatenated into a URL, path, or other structured string, either give the example in the exact normalized form the code requires, or state explicitly that the value gets normalized before use. An example doubles as an implicit test case implementers copy verbatim — if it's in the wrong shape, that shape ships.
...
### Gap: a useful lookup was built only as an error-path side effect, not as its own command
**Fixed in:** `v0.5.0`
**What was missing:** the `v0.3.0` fix for the previous gap ("no error-message quality bar") added a provider's available-model list to `compile`'s error message on a model-not-found failure — but nothing asked "should a user be able to get this list *before* guessing wrong, not just after?" The capability (`resolve_provider_target` + `genai::Client::all_model_names`) already existed end-to-end; it just wasn't reachable except by triggering the failure first.
**What it took:** a user who liked the error-path listing had to explicitly ask for it as its own `okf-mcp models <provider>` command (plus an `okf-list-models` MCP tool) — a thin CLI/MCP wrapper around logic that had been sitting fully built inside the error handler the whole time.
**Lesson:** when a fix builds a genuinely useful lookup/query as a side effect of handling an error, treat "should this also be its own first-class command?" as a required question before calling the fix done — not a follow-up to wait for someone to request. The cost of exposing it directly is usually small (the hard part, the actual lookup logic, is already written), and burying a generally useful capability inside an error path is what makes it invisible until someone happens to hit that specific failure.

### Gap: a common variant of a specified syntax was never enumerated
**Fixed in:** (this change — not yet released)
**What was missing:** `docs/okf-pipeline-design.md` specifies `[[concept-name]]` wikilinking and explicitly calls out the cross-vault variant (`[[personal::journal-entry]]`) as something `okf lint` must recognize and flag — but never mentions Obsidian's other extremely common wikilink variant, the piped alias (`[[target|display text]]`), despite the same doc stating wiki pages are "rendered natively in Obsidian with `[[wikilinks]]`."
**What it took:** `validator::wikilink::parse_link` special-cased `::` for cross-vault links but had no equivalent handling for `|` — any piped link's entire `target|display text` string was used as the lookup slug, which can never match a real filename. Every piped link in a live vault was unconditionally reported as a dangling link, discovered only when a user hand-wrote several and `okf-mcp lint` flagged concept pages that plainly existed on disk.
**Lesson:** when a design doc specifies a syntax by naming one non-default variant explicitly (here, cross-vault prefixing), audit the *other* variants the same underlying format is known to support upstream (here, Obsidian's own alias syntax) and either enumerate them too or state they're intentionally unsupported. Naming one variant creates an implicit (and misleading) impression of completeness that silently excludes siblings of the same feature.

### Gap: a validation layer that fully diagnoses a problem was never asked whether it should also repair it
**Fixed in:** (this change — not yet released)
**What was missing:** the design doc's validation table (Dangling Links / Provenance Audit / Orphan Detection / Contradiction Flag) specifies what `lint` checks and how it fails, but never poses the follow-up question for the subset of findings that have exactly one correct, mechanical repair — e.g. a `sources:` path that resolves once a missing `.md` extension is appended — versus the subset that inherently need judgment (a genuinely missing concept page, which page or link the LLM compiler already exists to write).
**What it took:** the same real-world lint run that surfaced the pipe-alias gap above also contained a `sources:` entry missing its `.md` extension — a bug class with one unambiguous fix, still requiring a human to notice, diagnose, and hand-edit it, because `lint` (by design, deliberately LLM-free) could report the problem precisely but had no path to act on it, and `compile`/`rebuild` (which already run the LLM and already know how to synthesize a concept page from context) had no equivalent path for a *linked-to-but-missing* concept either.
**Lesson:** whenever a design specifies a validation/diagnostic layer, explicitly split its findings into "mechanically fixable with certainty" (add a repair path with no judgment call, safe to automate) versus "requires content judgment" (route to whatever existing content-generation capability the system already has, gated behind explicit confirmation before anything is committed) — rather than leaving every finding as report-only by default. The gap is easy to miss because a pure validator "working as designed" looks complete on its own terms; the missing question is whether *adjacent* capabilities already in the system (here, the LLM compiler) should be wired to it.

### Gap: no resilience story for the compiler's own output violating the schema it was told to produce
**Fixed in:** (this change — not yet released)
**What was missing:** the design doc's compiler-layer example shows the exact frontmatter template the LLM is prompted to emit (`id: concept_microservices`, etc.), and separately specifies validation should "verify YAML frontmatter fields" — but nowhere considers that the LLM will sometimes deviate from the very schema it was just handed, in a near-miss way (observed in real usage: emitting `tid:` instead of `id:`). The validation layer's "surface as an error, not a silent skip" design (itself correct, and already documented for genuinely malformed pages) was never split from a second, harsher property it also has: one page failing to parse aborts evaluation of the *entire* vault, hiding every other page's findings behind it.
**What it took:** `lint_bundle` (and therefore the new `--fix`) would return a bare `Err` and refuse to report anything at all if even one page had this typo — not "list it as one finding among many," but "tell the user nothing until they manually find and fix that one file." `validator::fix::fix_id_field_typos` added a pre-pass, ahead of `lint_bundle`, that renames exactly this observed typo and verifies the rewrite actually fixes parsing before keeping it, so one drifted page no longer blocks visibility into the rest of the vault.
**Lesson:** when a design has the compiler prompt an LLM with an exact structural contract, explicitly plan for the LLM occasionally missing that contract in a near-miss, not-totally-malformed way — and separately, when a validator's error-vs-report policy is "surface structural breaks as errors" (a reasonable default), explicitly decide whether that means "abort the whole run" or "report this one file as broken and keep evaluating everything else" — the design doc's silence defaulted to the former, which is far more disruptive than the doc's own validation table (built around a list of findings, not an all-or-nothing gate) implies it intended.
