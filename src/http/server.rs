...
use crate::core::mcp_server::OkfServer;
...
/// `OkfServer::ingest` (via rmcp's `RequestContext::extensions`, which
...
pub async fn start_http_server(
    mcp_service_factory: impl Fn() -> Result<OkfServer, std::io::Error> + Send + Sync + 'static,
...
mod tests {
    ...
    async fn health_metrics_auth_gate_cors_and_server_bootstrap_are_exercised() {
        ...
        let result = start_http_server(
            move || Ok(OkfServer::new(config.clone(), auth_manager.clone())),
            &server_config,
            state.registry,
            "header",
